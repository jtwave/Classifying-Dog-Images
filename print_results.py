...
def print_results(results_dic, results_stats_dic, model, 
    ...
    if ((print_incorrect_dogs == True) and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] != results_stats_dic['n_images'])):
        for image_name, info_list in results_dic.items():
                if (sum(results_dic[image_name][3:]) == 1):
                        print(f"{image_name}, classifier label of misclassified dog: {results_dic[image_name][1]}")
    ...
    if ((print_incorrect_breed == True) and (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed'])):
        for image_name, info_list in results_dic.items():
                if (sum(results_dic[image_name][3:]) == 2 and results_dic[image_name][2] == 0):
                        print(f"{image_name}, classifier label of misclassified breed: {results_dic[image_name][1]}")
    None
